...
# create megamatrix to hold all P matrices
...
#### CHANGING MU ONLY, SIGMA CONSTANT ####
  ...
  for(iS in 1:length(S_list)) { #go through all recovery scenarios
    ...
    for(ip in 1:length(pset)) {
      ...
      for(j in 1:length(percent_change_vec)){
        ...
        new_mu <- mu*(1 + curr_percent) #shift mu by percent of interest
        #keep sigma the same
        upper_limit <- new_mu + (sigma/abs(xi)) #calculate upper limit of shifted GEV
        x <- seq(u, upper_limit) #x values above hurricane threshold to maximum wind speed
        cdf_x <- exp(-(1 + xi*((x - new_mu)/sigma))^(-1/xi)) #calculate cumulative distribution function for corresponding x values
        cdf_data <- as.data.frame(cbind(x, cdf_x)) #store in dataframe
...
#### CHANGING SIGMA ONLY, KEEP MU CONSTANT ####
# create megamatrix to hold all P matrices
bigP <- array(0, c(length(percent_change_vec), nstate, nstate))
  ...
  for(iS in 1:length(S_list)) { #go through all recovery scenarios
    ...
    for(ip in 1:length(pset)) {
      ...
      for(j in 1:length(percent_change_vec)){
        curr_percent <- percent_change_vec[j]
        new_sigma <- sigma*(1 + curr_percent) #shift sigma by percent of interest
        upper_limit <- mu + (new_sigma/abs(xi)) #calculate upper limit of shifted GEV
        x <- seq(u, upper_limit) #x values above hurricane threshold to maximum wind speed
        cdf_x <- exp(-(1 + xi*((x - mu)/new_sigma))^(-1/xi)) #calculate cumulative distribution function for corresponding x values
        cdf_data <- as.data.frame(cbind(x, cdf_x)) #store in dataframe
